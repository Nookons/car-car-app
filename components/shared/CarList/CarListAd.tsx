...
import {Separator} from "@/components/ComponentsProvider";
        ...
        <Link href={`/en/car/${carAd.id}`} key={carAd.id} className="block w-full">
            <Card>
                <CardTitle>
                    ...
                    <div>
                        <p className={`text-neutral-500 text-xs`}>{t(`on`)} {carAd.platform} {t(`from`)} {dayjs(carAd.post_date).format("YYYY-MM-DD HH:mm")}</p>
                    </div>
                </CardTitle>
                <hr/>
